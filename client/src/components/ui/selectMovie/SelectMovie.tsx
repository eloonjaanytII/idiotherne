...
interface SelectMovieProps {
    ...
    onChangeYear: (range: YearRange) => void;
...
const SelectMovie: React.FC<SelectMovieProps>= ({
  ...
  return (
    <div className='grid md:grid-cols-5 grid-cols-2 gap-2 pr-8 pl-8'>
        <select value={countries} className="select select-accent" onChange={e => onChangeCountry(Number(e.target.value))}>
        ...
        <select value={genres} className="select select-accent" onChange={e => onChangeGenre(Number(e.target.value))}>
        {genresList.map(item => (
