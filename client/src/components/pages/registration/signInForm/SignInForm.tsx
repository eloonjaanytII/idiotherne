...
interface SignInFormProps {
  handlerSubmit: (data: SignInFormFields) => void;
  loginError?: string;
}

const SignInForm: React.FC<SignInFormProps> = ({handlerSubmit, loginError}) => {

  const {
    register,
    handleSubmit,
    formState: {errors},
  } = useForm<SignInFormFields>({mode: "onBlur"});
  ...
  return (
    <form onSubmit={handleSubmit(handlerSubmit)} className='flex flex-col gap-3 w-[60%] m-auto text-2xl'> 
        ...
        {loginError && <p className='text-center text-md text-accent'>{loginError}</p>}
