...
const updateFilmFlags = async (req, res) => {
    ...
    if (updatedCount === 0) return next(createError(404, 'Фильм или запись пользователя не найдены'));
...
const getFilm = async (req, res, next) => {
    ...
    if (!film) return next(createError(404, "Фильм не найден"));
...
const getUserFilmWithScores = async (req, res, next) => {
    ...
    if (!filmList) return next(createError(400,  "Случилась какая-то ошибка с поиском фильмов по оценкам"));
...
const getUserFilmWithFavorite = async (req, res, next) => {
    ...
    const filmList = await Film.findAll({where: {kinopoiskId: kinopoiskIdList}})
    if (filmList.length === 0) return res.status(200).json([]);
...
const getUserFilms = async (req, res, next) => {
    ...
    if (userChainList.length === 0) return res.status(200).json([]);
